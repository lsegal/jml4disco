...
public PublicScanner(
	...
	this.jmlLevel = JmlConstants.JML_LEVEL_NO_JML;
}
//<jml-end id="?" />
	...
	try {
		while (true) { //loop for jumping over comments
			...
			switch (this.currentCharacter) {
				case '@' :
					// <jml-start id="nnts" />
					// TODO: does JML also accept @'s anywhere inside the comment.
					if (inJmlComment) 
						continue;
					// <jml-end id="nnts" />
/*					if (this.sourceLevel >= ClassFileConstants.JDK1_5) {
				...
				case '/' :
					if (!skipComments) {
						...
						if (test == 0) { //line comment 
							// <jml-end id="nnts" />
							// TODO: do the following only if JML support is enabled ... here and elsewhere.
							if (this.jmlLevel != JmlConstants.JML_LEVEL_NO_JML && getNextChar('@')) { // jmlLineComment
								this.inJmlComment = true;
								this.inJmlLineComment = true;
								continue;
							}
							// <jml-end id="nnts" />
						...
						if (test > 0) { //traditional and javadoc comment
							try { //get the next char
								...
								// <jml-start id="nnts" />
								// FIXME: maybe. Should this be a getNextChar() ... or should the getNextChar above use currentChar?
								if (this.jmlLevel != JmlConstants.JML_LEVEL_NO_JML && this.currentCharacter == '@') {
									inJmlComment = true;
									continue;
								}
								// <jml-end id="nnts" />
				...
				default :
					...
					if (c < ScannerHelper.MAX_OBVIOUS) {
						if ((ScannerHelper.OBVIOUS_IDENT_CHAR_NATURES[c] & ScannerHelper.C_IDENT_START) != 0) {
							// <jml-start id="nnts" />
							if (inJmlComment && jmlLevel < JmlConstants.JML_LEVEL_DBC) {
								int jmlToken = scanIdentifierOrKeyword();
								if (jmlToken == ITerminalSymbols.TokenNamepure) {
									// Skip "pure" but continue processing in case there are nullity modifiers.
									continue;
								} else if (jmlToken == ITerminalSymbols.TokenNameIdentifier && !inJmlCast
										|| jmlToken == ITerminalSymbols.TokenNameassert
										|| jmlToken == ITerminalSymbols.TokenNamejml_assert
										|| jmlToken == ITerminalSymbols.TokenNameprivate
										|| jmlToken == ITerminalSymbols.TokenNameprotected
										|| jmlToken == ITerminalSymbols.TokenNamepublic
										|| jmlToken == ITerminalSymbols.TokenNameint
										|| jmlToken == ITerminalSymbols.TokenNamelong
										|| jmlToken == ITerminalSymbols.TokenNameshort
										|| jmlToken == ITerminalSymbols.TokenNameboolean
										|| jmlToken == ITerminalSymbols.TokenNamevoid
										|| jmlToken == ITerminalSymbols.TokenNamechar
										|| jmlToken == ITerminalSymbols.TokenNamefloat
										|| jmlToken == ITerminalSymbols.TokenNamedouble
										|| jmlToken == ITerminalSymbols.TokenNamestatic
								) {
...
// <jml-start id="3" />
/**
* Searches for a JML keyword and returns the appropriate token - else -1 is returned to indicate
* nothing meaningfull was found.
*/
private int internalScanJmlKeyword(int index, int length, char[] data) {
	switch (data[index]) {
				...
				case 8 :
					if (data[++index] == 'o') // non_null
						...
						if ((data[index] == 'u') // nullable
							...
							return -1;
				case 19 :
					if (data[++index] == 'o') // non_null_by_default
						if ((data[++index] == 'n')
							&& (data[++index] == '_')
							&& (data[++index] == 'n')
							&& (data[++index] == 'u')
							...
							&& (data[++index] == 'l')
							&& (data[++index] == 't'))
							return TokenNamenon_null_by_default;
						else
							return -1;
					else
						if ((data[index] == 'u') // nullable_by_default
							&& (data[++index] == 'l')
							&& (data[++index] == 'l')
							&& (data[++index] == 'a')
							&& (data[++index] == 'b')
							...
							&& (data[++index] == 'l')
							&& (data[++index] == 't'))
							return TokenNamenullable_by_default;
						else
							return -1;
				default:
					return -1;
		...
		case 'p':
			switch (length) {
				case 4:
					if ((data[index] == 'u') // pure
							&& (data[++index] == 'r')
							&& (data[++index] == 'e'))
						return TokenNamepure;
					else
						return -1;
				default:
					return -1;
