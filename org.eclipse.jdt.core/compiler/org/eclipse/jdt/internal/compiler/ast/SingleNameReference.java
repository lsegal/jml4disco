...
public void generateAssignment(BlockScope currentScope, CodeStream codeStream, Assignment assignment, boolean valueRequired) {
	...
	switch (this.bits & ASTNode.RestrictiveFlagMASK) {
		case Binding.FIELD : // assigning to a field
			...
			// <jml-start id="nnts" />
			this.generateTestForNullityField(currentScope, codeStream, 
											 assignment.expression.resolvedType);
			// <jml-end id="nnts" />
		...
		case Binding.LOCAL : // assigning to a local variable
			...
			if (localBinding.resolvedPosition != -1) {
				...
				if (assignment.expression.constant != Constant.NotAConstant) {
					...
					assignment.expression.generateCode(currentScope, codeStream, true);
					// <jml-start id="nnts" />
					this.generateTestForNullityLocal(currentScope, codeStream);
					// <jml-end id="nnts" />
...
protected void generateTestForNullityField(BlockScope currentScope,
		CodeStream codeStream, TypeBinding _resolvedType) {
	/* empty */
}
// <jml-end id="nnts" />
}
