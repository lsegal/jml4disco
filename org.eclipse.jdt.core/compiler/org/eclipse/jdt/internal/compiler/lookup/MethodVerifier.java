...
void checkAgainstInheritedMethods(MethodBinding currentMethod, MethodBinding[] methods, int length, MethodBinding[] allInheritedMethods) {
	...
	currentMethod.overriddenInheritedMethods = collectOverriddenInheritedMethods(methods, length, overriddenInheritedMethods);
...
// <jml-start id="checkSupersNullity">
private MethodBinding[] collectOverriddenInheritedMethods(MethodBinding[] methods, int length, int[] overriddenInheritedMethods) {
	if (true)
		if (methods == null)
			return new MethodBinding[0];
		else
			return (MethodBinding[])methods.clone();
	...
	int count = length;
	for (int i=0; i<length; i++) {
		if (overriddenInheritedMethods[i] == -1)
			count--;
	}
