...
// <jml-start id="X" />
private static final char fileSeparator = java.io.File.separatorChar;
private static final String[] emptyStringArray = new String[0];

// not currently used
public String[] getClasspath() {
	List result = new ArrayList(this.binaryLocations.length);
	for (int i=0, length=this.binaryLocations.length; i < length; i++) {
		ClasspathLocation location = this.binaryLocations[i];
		if (location instanceof ClasspathJar) {
			ClasspathJar jarLocation = (ClasspathJar) location;
			String zipFilename = jarLocation.zipFilename;
			result.add(fileSeparator == '/' 
						? zipFilename 
				    	: zipFilename.replace('/', fileSeparator));
/*			IResource resource = jarLocation.resource;
			if (resource != null) {
				IPath fullPath = resource.getFullPath();
				result[i] = fullPath.toOSString();
			} else {
*/
		} else if (location instanceof ClasspathMultiDirectory) {
			ClasspathMultiDirectory mdLocation = (ClasspathMultiDirectory)location;
			IContainer folder = mdLocation.sourceFolder;
			if (folder != null) {
				IPath ipath_location = folder.getLocation();
				result.add(ipath_location.toOSString());
			}
			folder = mdLocation.binaryFolder;
			if (folder != null) {
				IPath ipath_location = folder.getLocation();
				result.add(ipath_location.toOSString());
			}
		} else if (location instanceof ClasspathDirectory) {
			ClasspathDirectory dir = (ClasspathDirectory)location;
			IContainer folder = dir.binaryFolder;
			if (folder != null) {
				IPath ipath_location = folder.getLocation();
				result.add(ipath_location.toOSString());
			}
		} else {
			String msg = "unhandled type of ClasspathLocation: "+location.getClass().toString(); //$NON-NLS-1$
			Logger.println("NameEnvironment.getClasspath(): " + msg); //$NON-NLS-1$
			RuntimeException e = new RuntimeException(msg);
			Logger.dumpStack(e);
			throw e;
		}
	}
	return (String[])result.toArray(emptyStringArray);
}
public String[] getSourcePath() {
	List result = new ArrayList();
	for (int i=0, length=this.sourceLocations.length; i < length; i++) {
		String path = this.sourceLocations[i].sourceFolder.getLocation().toOSString();
		result.add(path);
	}
	return (String[])result.toArray(emptyStringArray);
}
