...
// <jml-start id="2" />
// FIXME: there must be a way to reuse code from this class rather than creating our own methods.
public void newClassFromName(char[] className) {
	// new: java.lang.ClassCastException
	if (CodeStream.DEBUG) System.out.println(this.position + "\t\tnew: java.lang.ClassCastException"); //$NON-NLS-1$
	if (this.classFileOffset + 13 >= this.bCodeStream.length) {
		this.resizeByteArray();
	}
	this.position++;
	this.stackDepth+=2;
	this.bCodeStream[this.classFileOffset++] = Opcodes.OPC_new;
	this.writeUnsignedShort(this.constantPool.literalIndexForType(className));
	this.dup();
	this.invoke(
			Opcodes.OPC_invokespecial,
			0, // argCount
...
public void newClassFromName(char[] className, String msg) {
	// new: java.lang.ClassCastException
	if (CodeStream.DEBUG) System.out.println(this.position + "\t\tnew: java.lang.ClassCastException"); //$NON-NLS-1$
	if (this.classFileOffset + 13 >= this.bCodeStream.length) {
		this.resizeByteArray();
	...
	this.invoke(
			...
			className,
			ConstantPool.Init,
			"(Ljava/lang/String;)V".toCharArray()); //$NON-NLS-1$
}
// <jml-end id="2" />
// <jml-start id="debugRAC" />
public void getSystemDotOut() {
	if (DEBUG) System.out.println(position + "\t\tgetSystemDotOut"); //$NON-NLS-1$
	generateFieldAccess(
			Opcodes.OPC_getstatic,
			1, // returnTypeSize,
			ConstantPool.JavaLangSystemConstantPoolName,
			"out".toCharArray(), //$NON-NLS-1$
			ConstantPool.JavaIoPrintStreamSignature);
}
public void invokeSystemOutPrintln() {
	// invokestatic #21 <Method java/lang/System.arraycopy(Ljava/lang/Object;ILjava/lang/Object;II)V>
	if (DEBUG) System.out.println(position + "\t\tinvokevirtual: java.lang.System.out.println(Ljava/lang/String)V"); //$NON-NLS-1$
	this.invoke(
			Opcodes.OPC_invokevirtual,
			1, // argCount
			0, // return type size
			"java/io/PrintStream".toCharArray(),
			"println".toCharArray(),
			"(Ljava/lang/String;)V".toCharArray());
}
public void invokeStringBufferAppend(char[] signature)
{
	this.invoke(
		...
		"append".toCharArray(),
		("("+new String(signature)+")Ljava/lang/StringBuffer;").toCharArray());
}
