...
//		    && !rightIsStartOk
	...
	private String getOperator(VcOperator operator, TypeBinding operandType) {
		if (operator == VcOperator.EQUALS) {
			if (operandType == TypeBinding.BOOLEAN) {
				return "boolEq"; //$NON-NLS-1$
			} else {
				return "integralEQ"; //$NON-NLS-1$
			}
		}
		if (operator == VcOperator.NOT_EQUALS) {
			if (operandType == TypeBinding.BOOLEAN) {
		...
		if (operator == VcOperator.LESS) {
			if (Type.isIntegral(operandType)) {
				return "integralLT"; //$NON-NLS-1$
			}
		}
		if (operator == VcOperator.LESS_EQUALS) {
			if (Type.isIntegral(operandType)) {
				return "integralLE"; //$NON-NLS-1$
			}
		}
		if (operator == VcOperator.GREATER) {
			if (Type.isIntegral(operandType)) {
				return "integralGT"; //$NON-NLS-1$
			}
		}
		if (operator == VcOperator.GREATER_EQUALS) {
			if (Type.isIntegral(operandType)) {
				return "integralGE"; //$NON-NLS-1$
			}
		}
		if (operator == VcOperator.PLUS) {
			if (Type.isIntegral(operandType)) {
				return "+"; //$NON-NLS-1$
			}
		}
		if (operator == VcOperator.MINUS) {
			if (Type.isIntegral(operandType)) {
				return "-"; //$NON-NLS-1$
			}
		}
		if (operator == VcOperator.TIMES) {
			if (Type.isIntegral(operandType)) {
				return "*"; //$NON-NLS-1$
			}
		}
		if (operator == VcOperator.DIVIDE) {
			if (Type.isIntegral(operandType)) {
				return "integralDiv"; //$NON-NLS-1$
			}
		}
	    if (operator == VcOperator.REMAINDER) {
			if (Type.isIntegral(operandType)) {
				return "integralMod"; //$NON-NLS-1$
			}
		}
	    if (operator == VcOperator.IMPLIES) {
			if (Type.isBoolean(operandType)) {
				return "IMPLIES"; //$NON-NLS-1$
			}
		}
	    if (operator == VcOperator.EQUIV) {
			if (Type.isBoolean(operandType)) {
				return "boolEq"; //$NON-NLS-1$
			}
		}
	    if (operator == VcOperator.NOT_EQUIV) {
			if (Type.isBoolean(operandType)) {
				return "boolNE"; //$NON-NLS-1$
			}
		}
	...
	private String labelText(VC vc) {
		if ((vc.kindOfAssertion()==KindOfAssertion.LOOP_INVAR && vc.labelStart() == 0)
		 || (vc.kindOfAssertion()==KindOfAssertion.PRE && vc.labelStart() == 0))
