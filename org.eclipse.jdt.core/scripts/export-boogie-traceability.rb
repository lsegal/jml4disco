...
class Group
    ...
    def self.order
        ["stmt", "expr", "field", "decl", "lit", "jml", "call", "array", "javadoc", "misc"]
    end
end
...
class TableEmitter
    ...
    def self.emit_groups(o)
        ...
        Group.order.map do |group|
            i += 1
            TableGroupEmitter.emit(o, group, i)
        end.join("\n")
    end
end
...
class TableGroupEmitter
    def self.emit(o, grp, start)
        <<-eof
            ...
            #{emit_items(o, grp, start)}
        eof
    end

    def self.emit_items(o, grp, start)
...
class TableRowEmitter
    def self.emit(o, i)
        if o.priority.value == "0"
            ...
            <<-eof
                <tr style='#{!o.done && o.priority.colour ? "color:" + o.priority.colour : ""}'>
                    <td align='left'>#{i}</td>
