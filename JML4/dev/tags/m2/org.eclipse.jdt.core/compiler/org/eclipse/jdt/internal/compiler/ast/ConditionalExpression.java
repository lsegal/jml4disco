...
public FlowInfo analyseCode(BlockScope currentScope, FlowContext flowContext,
		...
		setNullStatus(valueIfTrue.nullStatus(trueFlowInfo), valueIfFalse.nullStatus(falseFlowInfo));
		// <jml-start id="jmlConditional" />	
...
public int nullStatus(FlowInfo flowInfo) {
	...
	public boolean isDeclaredNonNull() {
		return valueIfTrue.isDeclaredNonNull() && valueIfFalse.isDeclaredNonNull();
	}
	public boolean isDeclaredMonoNonNull() {
		boolean trueIsNonNull = valueIfTrue.isDeclaredNonNull();
		boolean falseIsNonNull= valueIfFalse.isDeclaredNonNull();
		if (trueIsNonNull && falseIsNonNull)
			return false;
		boolean trueIsMono  = valueIfTrue.isDeclaredMonoNonNull();
		boolean falseIsMono = valueIfFalse.isDeclaredMonoNonNull();
		return (trueIsMono && (falseIsMono || falseIsNonNull))
		    || (trueIsNonNull && falseIsMono);
	}
	// <jml-end id="2" />
	// <jml-start id="jmlConditional" />
