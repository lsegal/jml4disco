...
public abstract class OperatorExpression extends Expression implements OperatorIds {
	...
	public static int[][] OperatorSignatures = new int[NumberOfTables+NumberOfExtraTablesForJML][];
	...
	public static final void classInitialize() {
		...
		// <jml-start id="level.0.expression" />
		OperatorSignatures[JML_IMPLIES] = get_AND_AND();
		OperatorSignatures[JML_REV_IMPLIES] = get_AND_AND();
		// <jml-end id="level.0.expression" />
	}
	...
	public String operatorToString() {
		switch ((bits & OperatorMASK) >> OperatorSHIFT) {
			...
			case JML_REV_IMPLIES :
				return "<=="; //$NON-NLS-1$
			case JML_NONNULLELEMENTS :
				return "\\nonnullelements"; //$NON-NLS-1$
			case JML_NOT_EQUIV :
				return "<=!=>"; //$NON-NLS-1$
			case JML_OLD :
				return "\\old"; //$NON-NLS-1$
			case JML_PRE :
