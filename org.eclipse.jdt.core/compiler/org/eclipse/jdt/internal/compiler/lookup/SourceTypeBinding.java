...
public class SourceTypeBinding extends ReferenceBinding {
	...
	// <jml-start id="?" />
	public boolean jmlFieldsMergedIn = false;
	public boolean jmlMethodsMergedIn = false;
	// <jml-start id="?" />
...
public SourceTypeBinding(char[][] compoundName, PackageBinding fPackage, ClassScope scope) {
	...
	this.scope = scope;
	// <jml-start id="?" />
	this.typeDeclaration = this.scope.referenceContext;
	// <jml-end id="?" />
...
public FieldBinding[] fields() {
	if ((this.tagBits & TagBits.AreFieldsComplete) != 0)
		// <jml-start id="?" />
		return mergeDeclarationIfNeeded(this.fields);	
		// <jml-end id="?" />
...
// <jml-start id="?" />
private FieldBinding[] mergeDeclarationIfNeeded(FieldBinding[] fieldBindings) {
	if (!jmlFieldsMergedIn && scope != null && scope.compilerOptions() != null && scope.compilerOptions().jmlEnabled) {
		JmlBinaryLookup.mergeFields(fieldBindings, this.typeDeclaration.fields);
		jmlFieldsMergedIn = true;
	}
	return fieldBindings;
}
private MethodBinding[] mergeDeclarationIfNeeded(MethodBinding[] methodBindings) {
	if (!jmlMethodsMergedIn && this.scope != null && this.scope.compilerOptions() != null && this.scope.compilerOptions().jmlEnabled ) {
		JmlBinaryLookup.mergeMethods(scope.problemReporter(), methodBindings, this.typeDeclaration.methods);
		jmlMethodsMergedIn = true;
	}
...
public MethodBinding getExactMethod(char[] selector, TypeBinding[] argumentTypes, CompilationUnitScope refScope) {
	...
	if ((this.tagBits & TagBits.AreMethodsComplete) != 0) { // have resolved all arg types & return type of the methods
		// <jml-start id="?" />
		// FIXME: maybe we want to merge before some of the other return statements in this method.
		mergeDeclarationIfNeeded(this.methods);	
		// <jml-end id="?" />
		long range;
...
public MethodBinding[] methods() {
	if ((this.tagBits & TagBits.AreMethodsComplete) != 0)
		// <jml-start id="?" />
		return mergeDeclarationIfNeeded(this.methods);
		// <jml-end id="?" />
