...
public boolean compile(String[] argv) {
	...
	try {
		...
		if (this.proceed) {
			// <jml-start id="batch.options" />
			// The following is useful to see if our JML specific options are 
			// being processed correctly. Note that this code exists in the JDT,
			// though it was commented out; we simply uncommented it.
				if (this.verbose) {
					System.out.println(new CompilerOptions(this.options));
				}
			// <jml-end id="batch.options" />
	...
	while (++index < argCount) {
		...
		switch(mode) {
			case DEFAULT :
				...
				int i = org.jmlspecs.jml4.compiler.CompilerExtensionManager.configureArgs(currentArg,newCommandLineArgs,index,this.options);
				if (i >= index) {
				    index = i;
				    continue;
				}
...
protected void handleWarningToken(String token, boolean isEnabling) throws InvalidInputException {
	if (token.equals("constructorName")) { //$NON-NLS-1$
		...
		this.options.put(
				...
				isEnabling ? CompilerOptions.WARNING : CompilerOptions.IGNORE);	
	// <jml-start id="nnts" />
	} else if (org.jmlspecs.jml4.compiler.CompilerExtensionManager.handleWarningToken(token,isEnabling,this.options)) {
