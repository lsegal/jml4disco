...
//					: var.name +                     INC_SEP + var.incarnation);
	...
	public String visit(VcLogicalExpression vc) {
		...
		if (vc.operator.equals(VcOperator.IMPLIES) && vc.left instanceof VcAndNary)
			operator = ""; //$NON-NLS-1$
		String left = vc.left.accept(this);
		String right = vc.right.accept(this);
		if (vc.operator.equals(VcOperator.IMPLIES) && vc.left.endsInImpliesTrue())
			return right;
	...
	private String getOperator(VcOperator op, TypeBinding type) {
		if (op.equals(VcOperator.EQUALS) || op.equals(VcOperator.EQUIV)) {
			return "="; //$NON-NLS-1$
		} else if (op.equals(VcOperator.NOT_EQUALS) || op.equals(VcOperator.NOT_EQUIV)) {
			return "~="; //$NON-NLS-1$
		} else if (op.equals(VcOperator.IMPLIES)) {
			return "-->"; //$NON-NLS-1$
		} else if (op.equals(VcOperator.DIVIDE)) {
			return "div"; //$NON-NLS-1$
		} else if (op.equals(VcOperator.REMAINDER)) {
	...
	private String[] getBoundsFromLogicalExpression(VcLogicalExpression range) {
		if (range.operator.equals(VcOperator.IMPLIES)) {
	...
	private String getLowerBound(VcVarDecl boundVar, VC rangeLeft) {
		...
		if (! (var.name.equals(boundVar.name) && var.pos == boundVar.pos))
			return null;
		boolean isLessThan = (op.equals(VcOperator.LESS));
