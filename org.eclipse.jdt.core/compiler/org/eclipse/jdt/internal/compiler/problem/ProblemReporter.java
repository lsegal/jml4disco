...
public static long getIrritant(int problemID) {
	switch(problemID){
	    ...
	    // <jml-start id="2" />
	    case IProblem.MethodReturnsNull :
	    case IProblem.DereferenceNull :
	    case IProblem.DereferenceNullField :
	    case IProblem.AssignNull :
	    case IProblem.SendNull :
	    case IProblem.SendToNull :
	    case IProblem.UninitializedBlankNonNullField :
	    case IProblem.IncompatibleReturnTypeNullity :
	    case IProblem.IncompatibleParameterNullity :
		    return JmlCompilerOptions.ReportNonNullTypeSystem;
	    case IProblem.JmlDisabled: 
	    	return JmlCompilerOptions.ReportJmlCommentDisabled;
...
//		case IProblem.LocalVariableMayBeNull :
		...
		// <jml-start id="2" />
		case IProblem.TypeReferenceNotExplicitlyMarkedWithNullity :
...
public void nullityOnBaseType(TypeReference ref) {
	String[] arguments = new String[] {ref.toString(), "no nullity modifier"}; //$NON-NLS-1$
	this.handle(
...
public void typeReferenceNotExplicitlyMarkedWithNullity(TypeReference ref, String id) {
	String[] arguments = new String[] { new String(ref.toString()), new String(id)};
	this.handle(
		IProblem.TypeReferenceNotExplicitlyMarkedWithNullity,
		arguments,
		arguments,
		ref.sourceStart,
		ref.sourceEnd);
...
public void attemptToDereferenceNullValue(QualifiedNameReference qualifiedNameReference) {
	String[] arguments = new String[] { new String(qualifiedNameReference.tokens[1]), new String(qualifiedNameReference.tokens[1])};
	this.handle(
		IProblem.DereferenceNullField,
		arguments,
		arguments,
		qualifiedNameReference.sourceStart,
		qualifiedNameReference.sourceEnd);
}
public void attemptToDereferenceNullValue(QualifiedNameReference qualifiedNameReference, int index) {
	String[] arguments = new String[] { new String(qualifiedNameReference.tokens[index]), new String(qualifiedNameReference.tokens[index])};
	this.handle(
		IProblem.DereferenceNullField,
		arguments,
		arguments,
...
public void attemptToAssignNullValue(AbstractVariableDeclaration declaration) {
	String[] arguments = new String[] { new String(declaration.name) };	
	this.handle(
			IProblem.AssignNull,
			arguments,
			arguments,
...
public void attemptToSendNullValue(ExplicitConstructorCall constrCall, int i, String formalName) {
	String[] arguments = new String[] { Integer.toString(i), new String(formalName)};
	this.handle(
		IProblem.SendNull,
		arguments,
		arguments,
		constrCall.sourceStart,
		constrCall.sourceEnd);
}
public void attemptToSendNullValue(MessageSend messageSend, int i, String formalName) {
	String[] arguments = new String[] { Integer.toString(i), new String(formalName)};
	this.handle(
		IProblem.SendNull,
		arguments,
...
public void attemptToSendNullValue(AllocationExpression allocExp, int i, String formalName) {
	String[] arguments = new String[] { Integer.toString(i), new String(formalName)};
	this.handle(
		IProblem.SendNull,
		arguments,
		arguments,
		allocExp.sourceStart,
		allocExp.sourceEnd);
}
public void attemptToSendToNullValue(MessageSend messageSend) {
	String[] arguments = new String[] {new String(messageSend.selector)};
	this.handle(
		IProblem.SendToNull,
		arguments,
		arguments,
...
public void uninitializedBlankNonNullField(FieldBinding field, ASTNode location) {
	String[] arguments = new String[] {new String(field.readableName())};
	this.handle(
		IProblem.UninitializedBlankNonNullField,
		arguments,
		arguments,
		nodeSourceStart(field, location),
		nodeSourceEnd(field, location));
}
public void attemptBroadenReturnNullity(JmlMethodDeclaration methodDecl, TypeReference superType) {
	String[] arguments = new String[] {new String(methodDecl.selector), new String(superType.toString())};
	this.handle(
			IProblem.AttemptBroadenReturnNullity,
			arguments,
			arguments,
...
// <jml-end id="2" />
// <jml-start id="checkSupersNullity" />
public void incompatibleReturnTypeNullity(MethodDeclaration method, ReferenceBinding inheritedFrom) {
	String[] arguments = new String[] {new String(method.selector), new String(inheritedFrom.sourceName)};
	this.handle(
			IProblem.IncompatibleReturnTypeNullity,
			arguments,
			arguments,
			method.returnType.sourceStart,
			method.returnType.sourceEnd);
}
public void incompatibleParameterNullity(Argument currentsArg, ReferenceBinding inheritedFrom) {
	String[] arguments = new String[] {new String(currentsArg.name), new String(inheritedFrom.sourceName)};
	this.handle(
			IProblem.IncompatibleParameterNullity,
			arguments,
			arguments,
			currentsArg.type.sourceStart,
			currentsArg.type.sourceEnd);
}
//<jml-end id="checkSupersNullity" />

// <jml-start id="jmlComments" />
public void jmlDisabled(int sourceStart, int sourceEnd) {
	this.handle(
			IProblem.JmlDisabled,
			NoArgument,
			NoArgument,
			sourceStart,
			sourceEnd);
...
//<jml-start id="modifiers" />
public void invalidModifier(String where, String kind, String allowed, JmlTypeBodyDeclaration decl) {
	String[] arguments = new String[] {where, kind, allowed};
	this.handle(
		IProblem.InvalidModifier,
		arguments,
		arguments,
		decl.modifiersSourceStart,
		decl.sourceStart - 2);
}
public void invalidModifier(String where, String kind, String allowed, int start, int end, TypeDeclaration decl) {
	this.referenceContext = decl;
	String[] arguments = new String[] {where, kind, allowed};
	this.handle(
		IProblem.InvalidModifier,
		arguments,
...
public void duplicateModifier(String where, String kind, JmlTypeBodyDeclaration decl) {
		String[] arguments = new String[] {where, kind};
		this.handle(
			IProblem.DuplicateModifier,
			arguments,
			arguments,
			decl.modifiersSourceStart,
			decl.sourceStart - 2);
	}
//<jml-end id="modifiers" />

// <jml-start id="escsupport1" />
public void jmlEsc2Caution(String msg, int sourceStart, int sourceEnd) {
	
	String[] arguments = new String[] {msg};
	this.handle(IProblem.JmlEsc2Caution, arguments, arguments, sourceStart, sourceEnd);
}
public void jmlEsc2Warning(String msg, int sourceStart, int sourceEnd) {
	
	String[] arguments = new String[] {msg};
	// we store the referenceContext because we don't want esc warnings to cause compilation to abort
	ReferenceContext refContext = this.referenceContext;
	this.handle(IProblem.JmlEsc2Warning, arguments, arguments, sourceStart, sourceEnd);
	this.referenceContext = refContext;
}
public void jmlEsc2Error(String msg, int sourceStart, int sourceEnd) {
	
	String[] arguments = new String[] {msg};
	// we store the referenceContext because we don't want esc errors to cause compilation to abort
	ReferenceContext refContext = this.referenceContext;
	this.handle(IProblem.JmlEsc2Error, arguments, arguments, sourceStart, sourceEnd);
	this.referenceContext = refContext;
}
public void jmlEsc2Fatal(String msg, int sourceStart, int sourceEnd) {
	
	String[] arguments = new String[] {msg};
	this.handle(IProblem.JmlEsc2Fatal, arguments, arguments, sourceStart, sourceEnd);
}
// <jml-end id="escsupport1" />
...
public int computeSeverity(int problemID){
	...
	switch (problemID) {
		...
		case IProblem.JavadocMissingTagDescription:
			...
			break;
		// <jml-start id="jml2-and-esc2-problem-reporting"/>
		case IProblem.JmlEsc2Caution:
		case IProblem.JmlEsc2Warning:
			return ProblemSeverities.Warning;
		case IProblem.JmlEsc2Error:
		case IProblem.JmlEsc2Fatal:
			return ProblemSeverities.Error;
		// <jml-end id="jml2-and-esc2-problem-reporting"/>
	}
	long irritant = getIrritant(problemID);
	if (irritant != 0) {
...
public void illegalExpressionForSetMembership(Expression expression, LocalDeclaration declaration) {
	this.handle(
		IProblem.IllegalExpressionForSetMembership, 
		new String[]{ new String(declaration.name) }, 
		new String[]{ new String(declaration.name) }, 
		expression.sourceStart, 
		expression.sourceEnd);
}
public void illegalTypeForSetComprehension(TypeReference typeReference) {
	this.handle(
			IProblem.IllegalTypeForSetComprehension, 
			NoArgument,
			NoArgument,
...
// <jml-start id="assignable" />
public void illegalUsageOfThis(TypeReference thisRef) {
	this.handle(
			IProblem.InvalidUsageOfThis,
			NoArgument, 
			NoArgument, 
			thisRef.sourceStart,
			thisRef.sourceEnd);
}
// <jml-end id="assignable" />
...
public void invalidSuperAsStoreRef(TypeReference superRef) {
	this.handle(
			IProblem.InvalidStoreRefUsage,
			NoArgument, 
			new String[] { "super" },   //$NON-NLS-1$
			superRef.sourceStart,
			superRef.sourceEnd);
}
...
private boolean isKeyword(int token) {
	switch(token) {
		...
		// FIXME: note that non_null and nullable can also be identifiers.
		case TerminalTokens.TokenNamealso:
		case TerminalTokens.TokenNamejml_assert:
		case TerminalTokens.TokenNamecode_bigint_math:
		case TerminalTokens.TokenNamecode_java_math:
		case TerminalTokens.TokenNamecode_safe_math:
		case TerminalTokens.TokenNameghost:
		case TerminalTokens.TokenNamehelper:
		case TerminalTokens.TokenNameinstance:
		case TerminalTokens.TokenNamemodel:
		case TerminalTokens.TokenNamenon_null:
		case TerminalTokens.TokenNamenon_null_by_default:
		case TerminalTokens.TokenNamenullable:
		case TerminalTokens.TokenNamenullable_by_default:
		case TerminalTokens.TokenNamepure:
		case TerminalTokens.TokenNamespec_public:
		case TerminalTokens.TokenNamespec_protected:
		case TerminalTokens.TokenNamespec_bigint_math:
		case TerminalTokens.TokenNamespec_java_math:
		case TerminalTokens.TokenNamespec_safe_math:
		case TerminalTokens.TokenNameuninitialized:
		case TerminalTokens.TokenNameAssignableOrSynonym:
		case TerminalTokens.TokenNameAssignableRedundantlyOrSynonym:
		case TerminalTokens.TokenNameaxiom:
		case TerminalTokens.TokenNameBehaviorOrSynonym:
		case TerminalTokens.TokenNameconstraint:
		case TerminalTokens.TokenNamediverges:
		case TerminalTokens.TokenNameEnsuresOrSynonym:
		case TerminalTokens.TokenNameEnsuresRedundantlyOrSynonym:
		case TerminalTokens.TokenNameinitially:
		case TerminalTokens.TokenNameinvariant:
		case TerminalTokens.TokenNameinvariant_redundantly:
		case TerminalTokens.TokenNamerepresents:
		case TerminalTokens.TokenNameRequiresOrSynonym:
		case TerminalTokens.TokenNameRequiresRedundantlyOrSynonym:
		case TerminalTokens.TokenNameSignalsOrSynonym:
		case TerminalTokens.TokenNameSignalsRedundantlyOrSynonym:
		case TerminalTokens.TokenNamesignals_only:
		case TerminalTokens.TokenNameslash_everything:
        case TerminalTokens.TokenNameslash_exists:
        case TerminalTokens.TokenNameslash_forall:
        case TerminalTokens.TokenNameslash_fresh:
        case TerminalTokens.TokenNameslash_max:
        case TerminalTokens.TokenNameslash_min:
        case TerminalTokens.TokenNameslash_nonnullelements:
        case TerminalTokens.TokenNameslash_not_specified:
        case TerminalTokens.TokenNameslash_nothing:
        case TerminalTokens.TokenNameslash_num_of:
        case TerminalTokens.TokenNameslash_old:
        case TerminalTokens.TokenNameslash_peer:
        case TerminalTokens.TokenNameslash_pre:
        case TerminalTokens.TokenNameslash_product:
        case TerminalTokens.TokenNameslash_readonly:
        case TerminalTokens.TokenNameslash_rep:
        case TerminalTokens.TokenNameslash_result:
        case TerminalTokens.TokenNameslash_same:
        case TerminalTokens.TokenNameslash_sum:
        case TerminalTokens.TokenNameslash_type:
		// <jml-start id="level.0-1-a" />
			return true;
		default: 
			return false;
...
public void parseError(
	...
	// <jml-start id="slashTokens" />
	final String slash_ = "slash_"; //$NON-NLS-1$
	final String cSlash = "\\";     //$NON-NLS-1$
	final int slash_length = slash_.length();
	if (errorTokenName.startsWith(slash_)) {
		errorTokenName = cSlash + errorTokenName.substring(slash_length);
	}
	for (int i = 0; i < possibleTokens.length; i++) {
		if (possibleTokens[i].startsWith(slash_)) {
			possibleTokens[i] = cSlash + possibleTokens[i].substring(slash_length);
		}
	}
	// <jml-end id="slashTokens" />
...
private void syntaxError(
	...
	// <jml-start id="slashedKeywords" />
	final String sSlash = "\\"; //$NON-NLS-1$
	final String SLASH  = "slash_"; //$NON-NLS-1$
	final int SLASH_length = SLASH.length();
	if (expectedToken != null && expectedToken.startsWith(SLASH))
		expectedToken = sSlash+expectedToken.substring(SLASH_length);
	// <jml-end id="slashedKeywords" />
...
public void unnecessaryCast(CastExpression castExpression) {
	// <jml-start id="2" />
	// FIXME: (prj) I don't like this test here, but it's the best place I could find for it,
	//         since this is called from many places
	if (castExpression instanceof JmlCastExpression) {
		// FIXME: (prj) the following if condition is not enough.
		if (((JmlCastExpression)castExpression).isExplicitNonNull)
			return;
	}
	// <jml-end id="2" />
