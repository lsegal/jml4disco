...
//					}
		...
		// <jml-start id="implicationOperators" />
			break;
		//TODO: factor these out into a JmlEqualExpression class
		case JML_IMPLIES:
			generateImplies(currentScope, codeStream, valueRequired);
			break;
		case JML_REV_IMPLIES:
			generateRevImplies(currentScope, codeStream, valueRequired);
			break;
		// <jml-end id="implicationOperators" />
	}
	if (valueRequired) {
		codeStream.generateImplicitConversion(this.implicitConversion);
	}
...
// <jml-start id="implicationOperators" />
private void generateImplies(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
	Constant condConst;
	if ((this.left.implicitConversion & TypeIds.COMPILE_TYPE_MASK) == TypeIds.T_boolean) {
		if ((condConst = this.left.optimizedBooleanConstant()) != Constant.NotAConstant) {
			if (condConst.booleanValue() == false) {
				// <something equivalent to false> ==> x
				this.left.generateCode(currentScope, codeStream, false);
				this.right.generateCode(currentScope, codeStream, false);
				if (valueRequired) {
	...
	// default case
	BranchLabel endifLabel = new BranchLabel(codeStream);
	BranchLabel evaluateRightLabel = new BranchLabel(codeStream);
	this.left.generateCode(currentScope, codeStream, valueRequired);
	if (valueRequired) {
		codeStream.ifne(evaluateRightLabel);
		codeStream.iconst_1();
		codeStream.goto_(endifLabel);
		evaluateRightLabel.place();
	}
	this.right.generateCode(currentScope, codeStream, valueRequired);
	if (valueRequired) {
		endifLabel.place();
...
private void generateRevImplies(BlockScope currentScope, CodeStream codeStream, boolean valueRequired) {
	Constant condConst;
	if ((this.left.implicitConversion & TypeIds.COMPILE_TYPE_MASK) == TypeIds.T_boolean) {
		if ((condConst = this.left.optimizedBooleanConstant()) != Constant.NotAConstant) {
			if (condConst.booleanValue() == false) {
	...
	this.left.generateCode(currentScope, codeStream, valueRequired);
	if (valueRequired) {
		codeStream.ifeq(evaluateRightLabel);
		codeStream.iconst_1();
		codeStream.goto_(endifLabel);
		evaluateRightLabel.place();
	}
