...
public class Cvc3Visitor extends ProverVisitor {
	...
	public String visit(VcLogicalExpression vcExpr) {
		...
		if (vcExpr.operator.equals(VcOperator.EQUIV)) {
			op = "<=>"; //$NON-NLS-1$
		}
		if (op != null)
			return "(" + left + " "+op+" " + right + ")"; //$NON-NLS-1$//$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$

		Utils.assertTrue(vcExpr.operator.equals(VcOperator.NOT_EQUIV), "unknown logical operator '"+vcExpr.operator+"'"); //$NON-NLS-1$ //$NON-NLS-2$
